...
--centerlaunch = spawn "exec ~/.config/eww/dashboard/launch_dashboard" 
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
	...
	-- lauch password retrieve
    , ((modm .|. shiftMask, xK_p     ), spawn "~/bin/passScript.sh")
	
	-- set audio to external speaker
	-- , ((modm,				xK_F10   ), spawn "pulseaudio-ctl mute yes && pacmd set-sink-port 0 analog-output-headphones && pulseaudio-ctl mute no")

	-- set audio to internal(headphones)
	-- , ((modm,				xK_F11   ), spawn "pulseaudio-ctl mute yes && pacmd set-sink-port 0 analog-output-speaker && pulseaudio-ctl mute no")

	-- full screen focused window
	,((modm, xK_F11), withFocused toggleFloat)
    --, ((modm, xK_F12), centerlaunch)
    ...
    , ((0,                    xF86XK_AudioRaiseVolume), spawn "pulseaudio-ctl up 5")
    , ((0,                    xF86XK_AudioLowerVolume), spawn "pulseaudio-ctl down 5")
    , ((0,                    xF86XK_AudioMute), spawn "pactl set-sink-mute 0 toggle")

    -- Brightness keys
    , ((0,                    xF86XK_MonBrightnessUp), spawn "backlight_control +1")
    , ((0,                    xF86XK_MonBrightnessDown), spawn "backlight_control -1")
...
myStartupHook = do
  ...
  spawnOnce "feh --bg-scale ~/.config/wallpapers/Xmonad-wallpaper.png"
...
defaults = def {
      ...
      -- hooks, layouts
        ...
        layoutHook = gaps [(L,10), (R,10), (U,40), (D,10)] $ spacingRaw True (Border 5 5 5 5) True (Border 10 10 10 10) True $ smartBorders $ myLayout,
